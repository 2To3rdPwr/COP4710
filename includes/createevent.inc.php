<?php
    ...
    if(isset($_POST['submit']))
    {
        ...
        $sql = "SELECT * FROM event WHERE location = '$location' AND date= '$date' AND university_id IN (SELECT university_id FROM university where name = '$university_name')";
        $result = mysqli_query($conn, $sql) or trigger_error("Query Failed! SQL: $sql - Error: ".mysqli_error($conn), E_USER_ERROR);
        $resultcheck = mysqli_num_rows($result);
        
        if($resultcheck > 0)
        {
            header("Location: ../homepage.php?eventlocationoccupied");
        }
        else
        {
                    ...
                    else
                    {
                        ...
                        if($rso_name = 'None')
                        {
                            ...
                            if($user_privilege == '0')
                            {
                                ...
                                header("Location: ../homepage.php?success=eventToAdmin");
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            {
                                $university_id = $_SESSION['university_id'];
                                $sql = "INSERT INTO event (university_id, name, location, description, date, rso_id, approved, privacy) VALUES ('$university_id', '$name', '$location', '$description', '$combinedDT', '$rso_id', '0', $privacy )";
                                mysqli_query($conn, $sql) or trigger_error("Query Failed! SQL: $sql - Error: ".mysqli_error($conn), E_USER_ERROR);
                                header("Location: ../homepage.php?success=eventToAdmin");
                            }
                            else
